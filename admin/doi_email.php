...
<style>
  ...
  <?php 
    ...
    // lay du lieu do vao tai khoan va Kiem tra Bien
         $query_m="SELECT taikhoan,sodt,email FROM db_user WHERE id={$_SESSION['uid']}";
            ...
            if(!empty($error))
            {
                ...
                if(!empty($error2))
                {
                ...
                else 
                    {
                        if(!empty($error3))
                        {
                              $message2="<p class='require'>Số Điện Không Đúng</p>";
                        }
                        else {
                               if(!empty($error4))
                               {
                                   $message3 ="<p class='require'>Email  Đã Tồn Tại Trên Hệ Thống</p>";
                               }
                                else {
                                 // lay mat khau tren dua tren id cua Userid ng dang nhap hien tai
                                  $query_mk="SELECT id,matkhau FROM db_user WHERE matkhau=md5('{$_POST['matkhaucu']}') AND id={$_SESSION['uid']}";
                                   ...
                                   if(mysqli_num_rows($result_mk)!=1)
                                   {
                                    ...
                                    {
                                        $query_up_email="UPDATE db_user SET email='{$_POST['email']}' WHERE id={$_SESSION['uid']} ";
                                                           $result_up_email= mysqli_query($dbc,$query_up_email);
                                                           kt_query($result_up_email, $query_up_email);
                                            $message4= "<p class='thanhcong'>Đổi Email Thành Công</p>";
                                   }  
                            }    
                        }
    ...
    ?>
            ...
            <div class="box-header with-border">
              <h2 class="them">Đổi Email Đăng Ký</h2>
            </div>
            <!-- /.box-header -->
            <!-- form start -->
...
//                      
                  ...
                  <div class="col-sm-10">
                      <input type="text" class="form-control" minlength="6"  name="sodt" id="sodt" placeholder="Nhập Số Điện Thoại Đăng Ký" value=""/>
                   <?php 
                  ...
                  <div class="form-group">
                  <label for="inputPassword3" class="col-sm-2 control-label">Email Mới</label>
